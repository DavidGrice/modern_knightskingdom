...
- **CLN-03 done — #225** — `game/store/persistence.ts` (245 lines): one `PERSISTED_FIELDS` table (one row per persisted key, `satisfies`-checked so `tsc` rejects a missing row, an unknown row, or a wrongly typed default), a derived `PERSISTED_KEYS`, an explicit `LEGACY_LOAD_OVERRIDES` (exactly `landTier`, `cedricCaptures`, `cedricCapturedAtDay` — deliberately *not* folded into the fresh defaults), and `freshPersisted`/`applySave`/`buildSave`. `GameState` now `extends PersistedState`, so a field can no longer exist in the save but not in the store; adding a persisted field is two edits (`SaveGame` + one table row) instead of six. `gameStore.ts` 5064 → 4713 lines. **No `saveMigrations.ts` was created, on purpose**: `LEGACY_LOAD_OVERRIDES` *is* the migration, and a separate pre-pass would evaluate `cedricCapturedAtDay` against already-migrated values. **Audit correction:** there was no key-set drift to fix — all 66 persisted keys were present in all six places (the "drift" the audit cited was historical); the real duplication was the six hand-maintained lists themselves. Proven behavior-identical, not just type-safe: differential runs of a baseline server built from `main` versus the branch, in lockstep with `Date.now`/`Math.random` stubbed — initial state, `newGame` ×5, new-game-plus, a rich scripted session, ~480 load fixtures (each field dropped/nulled, legacy variants, a minimal save, unknown keys, non-null `destination`), 0 mismatches over 501 steps / ~1000 records, and a real guest save → reload → continue flow identical on both; the harness was itself shown able to fail (8 deliberate mutations, all caught); `toSave()`'s wire key order is unchanged (re-checked independently from `main`'s AST). **Left exactly as they were, for an owner decision (none changed):** `movingBuilding`/`carriedKeepExtra` still unsaved; `dirty`/autosave cadence untouched; dead `stabled`/`mounts` store fields and `SaveGame.playerPos` kept; session reset untouched (CLN-04). **Real issues surfaced, not fixed here:** (1) `MainMenu.describe` reads `cedricCaptures ?? 0` and ignores the Wave-38 inference `loadFromSave` applies, so a pre-Wave-38 save's slot card can show a lower rank than the loaded game; (2) a save missing `buildings` or `inventory` throws *after* `set()` has already applied (`seedNodes` reads `inventory`) — same as before; (3) `version` is written but never checked on load; (4) `ZERO_STATS`' nested objects are shared across fresh states (safe today only because every writer spreads them); (5) after a load the leaf and store `waterworks` alias one array, after `newGame` they are two separate ones.
- **CLN-04 done** — `game/store/sessionReset.ts` (59 lines: `resetSessionModules(next)`, the old leaf-reset list plus the new resets below) and `game/store/sessionHooks.ts` (22 lines, zero imports: `onSessionReset`/`runSessionResetHooks`). **Seam correction:** `onSessionReset` did not exist anywhere in `src/` (only in this file's prose); it is created here because `combat.ts`/`cedricSiege.ts` import `gameStore` and so cannot be imported from `sessionReset.ts` without re-closing the historical TDZ cycle. `newGame`, `startNewGamePlus` and `loadFromSave` are now thin callers of one `beginSession()` in `gameStore.ts` (order unchanged: module resets → `stabledHorses` → `setWaterworks` → `set()` → `worldEnv` → id-counter recompute (load only) → `seedNodes`), and `ephemeralState()` replaces the hand-written transient half (adds `photoMode`, `blueprintSelection` and the six build-view fields). `AgentManager` gains `clearHooks`, fired from `clear()`; Locomotion (`steerState`/`anchorCache`), `perception/state.ts` and `ai/actions/combatState.ts` (`states` + `alarmedAt`) register — the plan named two registrants, there were three. **Every reset was reproduced leaking on a baseline copy of `main` before it was added:** `placeHistory` (undo removed a *loaded* save's building), `carriedKeepExtra` (script-only: `finishMove` planted the old keep in the new save), `photoMode`, `blueprintSelection`, `defenderState` (a ghost defender soaked a raider's hits in a new game; a reused id came back at hp 3), `defenderOrders` + overrides + `scoutReported`, `ridingState`/`horses[id].mounted`, `villagerCombatState`, and the AI maps above (156 negative-confidence beliefs per reload from a stale `lastPerceiveAt`; a stale `engage` entry). **Leaks the plan missed, all reproduced on baseline:** the enemy store (a raider survives Continue and New Journey, plus its ram/ladder), player HP/stamina (3.5 HP after a reload; hp far above maxHp after a New Journey from a leveled session — refilled via a `refillVitals` flag the existing `combat.ts` store subscriber consumes once `maxHp` is re-derived), `dragonAir`/`dragonAirBlack` (quit mid-siege + Continue left `hostile` true: every villager in `flee_to_safety` forever and no dragon ever rolling again — the worst one), `cedricWarState` (its `active` flag gates all future sieges), and the arena run/badge (`endArenaRun()`). **Plan claims deliberately NOT actioned:** `fishingState` (true but harmless — `tickFishing` resets it within a frame and the dock is ~60 m from spawn), `statsAccum` (true but harmless — one ≤4 s flush window, credited to the same save; a reset would drop legitimate distance), `lastJoustAt` (a 3 s `performance.now` throttle), and the `buildSeq`/`villagerSeq`/`blueprintSeq`/`waterSeq` reset on newGame/NG+ (no visible effect; it would *cause* id reuse across games, and `companion.ts`, `villagerMobs`/`npcMobs` and the horse registry rely on fresh ids — counters stay recomputed on load only). **Mounted-component safety, traced per reset:** registries that a mounted component holds by reference are reset **in place** (`defenderState` entries whose id is a defender in the new roster — `DefenderFigure` registers in `useMemo([villager.id])` and would be orphaned by a delete; `villagerCombatState`, `ridingState`, `horses[id]`, `defenderOrders`, `dragonAir*`), and only entries no figure will ever own again are deleted; proven live (a same-id defender/villager still recovers from downed after a scripted load with the scene mounted, and enemies still hit the entry). **Proof:** headless-GPU-Chrome leak matrix (real UI Save & Return → Continue / New Journey, a real dragon siege, the arena, and scripted loads with the scene mounted) — baseline `main`: 28 leak rows FAIL; branch: 34/34 pass on the dev server *and* on `next build` + `next start`, 0 page errors; persisted-state differential (initial, newGame ×3, NG+, a rich session, ~144 load fixtures, `toSave()`, `worldEnv`, stable, id counters; `Date.now`/`Math.random` stubbed) baseline vs branch **byte-identical over 149 steps**, both throwing at the same 4 fixtures with identical messages, and a deliberate mutation (newGame's start time) was caught at 145 steps. **Intended behavior differences:** a same-save Continue no longer resumes wounds, defender orders, the saddle, raiders, the undo stack, `photoMode`, or an arena run. **Known hazards, left as they were:** a *scripted* `loadFromSave` mid-raid with the game scene mounted clears the enemy store while `Enemies.tsx`'s `raidActive` ref is still true and so fires its one-shot "raid beaten back" reward (measured +60 combat xp, +4 plank; the real UI unmounts the scene at load, so it is unreachable there — the same quirk as the existing knock-out path in `damagePlayer`; fixing it is a component change, CLN-13 territory); a scripted mid-siege load leaves the mounted siege runner flying (as on main); `Enemies.tsx` also targets a *reassigned* defender's stale `defenderState` entry in-session (a one-line roster filter, CLN-13). **Left for an owner decision, untouched:** world-sim ticks in build mode, `dirty`/autosave cadence, saving `movingBuilding`/`carriedKeepExtra`, deleting `stabled`/`mounts`/`playerPos`. **Follow-up commit (found by the independent verifier, then fixed and re-proven):** (a) `cartState`/`cartLivePos` were never reset, so a held ram or hitched cart survived Save & Return + Continue and a later let-go committed the *spawn* position into the save — `resetCarts()` (in place, `carts.ts` stays a leaf) now runs with the other resets; (b) `resetDefenders` refilled a reused id to its *old* `maxHp`, so a defender whose level/gear differs in the newly loaded save came back above or below its real ceiling (30/24, 24/36) — it now restores the fresh-registration shape (`hp = 1`) so `DefenderFigure`'s existing render-time refill applies. **That fix depends on the figure re-rendering after the reset**, which holds because the session reset's `useEnemyStore.clear()` replaces the `enemies` array every figure subscribes to (shown live: 0 frames observed at hp ≤ 1 over 40 scripted loads); a scratch negative control where `clear()` was made a no-op left a defender at 1/30 — so keep `clear()` replacing that array, or compute `maxHp` in the reset instead. **Design call worth an owner glance:** because HP/stamina were never saved, a same-tab Save & Return → Continue is now a full heal and abandons an in-progress raid without its reward (a real page reload or login already did both); the alternative (keeping a previous save's HP on a *different* save, or hp far above `maxHp` after a New Journey) is the actual bug, so the reset is the only coherent option, but it is a player-visible change. **Known remaining gaps, not fixed:** `crewState.engineId` (`game/crew.ts`) is likely not reset on a session switch (code reading only, not reproduced); `resetVillagerCombat` refills a reused villager id to its *old* `maxHp` and `Villagers.tsx` only clamps downward, so improved gear can leave one slightly below its new maximum.
- **CLN-05 done** — `src/lib/debugHooks.ts` (`exposeDebug(name, value)`: a `typeof window` guard plus the one assignment; **zero imports**, so the 12 previously import-free leaf modules — `riding`, `carts`, `env`, `buildCam`, `workSignal`, … — can use it without touching any import cycle). All **75** hand-rolled `window.__kk*` publications in **56** files were converted by a TypeScript-AST codemod (not a regex) — the plan's "~56" and "74" were both slightly off: the count is 75 because the audit's own regex missed the bare `__kk` on `gameStore.ts`, and 56 is the *file* count. Handled forms: `(window as unknown as Record<string, unknown>).__kkX = v`, the typed cast in `lib/audio.ts`, and the `w.__kkX` / `if (w) w.__kkX` alias forms in `combat.ts` and `PlayerController.tsx` (the now-unused `const w` aliases deleted). `if (typeof window !== 'undefined')` wrappers that held nothing but hook assignments were unwrapped (the helper guards itself); the two that also hold real client-only work (`collisionShapes.ts`' `loadCollisionShapes()`, `difficulty.ts`' initial `refresh()`) keep their guard. **Hook names, values and program positions are unchanged**, and no `NODE_ENV` gating was added (still an open owner decision below). Proof: a name-and-value-expression equivalence check over the TypeScript AST of `main` vs the branch (75/75 identical; mutation-checked — a changed value and a renamed hook are both caught), `tsc` and `next build` clean, a headless-GPU-Chrome key-set differential of production builds of `main` vs the branch (menu → new game → travel → second destination → home → arena: 68 hooks at the menu, 75 in game, identical key sets and identical deterministic call results in all 6 states, 0 page errors), the same differential under `next dev`, and three independent skeptic reviews (semantics of all 56 sites incl. a TS-AST "remove-the-hooks-and-diff-the-rest" check: 56/56 files identical; completeness/leftovers; `next dev` + HMR). **Notes for the next reader:** the 9 hook names the local `scripts/` tree references but `src/` never exposes (`__kkghostVerify`, `__kkbuildregion`, `__kklastNotify`, `__kkFPSinv`, `__kkpopSkipDebug`, `__kkplaceVerify`, `__kkholdTrace`, `__kkbolts`, `__kkTHREE`) are absent on `main` too — script-injected or long-dead, not a regression; `debugHooks.ts`' header carries a hand-written per-module index (machine-checked against the call sites when written — keep it current when adding a hook, or `grep -rn "^ *exposeDebug('" src` for the live list); editing `debugHooks.ts` under `next dev` re-creates ~70 modules' singletons, so reload the page afterwards (pre-existing analogue: HMR-editing `env.ts` leaves `__kkenv` disconnected from the live clock, identical on `main`).
