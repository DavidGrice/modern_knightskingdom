...
/** NPC_AI_SPEC §11.2 (Wave 42, E6) — "timestamped records with a
 *  natural-language `summary` string generated by a template". The TYPE
 *  lives here rather than in `core/Memory.ts`, matching this file's own
 *  established convention for `Belief` above (see `perception/Belief.ts`'s
 *  own header on why the type and the operations that write it live in
 *  different files) — `core/Memory.ts` owns push/cap/recall and the real
 *  producers, this just declares the shape both it and `Agent.ts` need to
 *  agree on. `at` is `AgentManager.now` (the AI clock), matching every other
 *  timestamp already on this Blackboard. */
...
export interface Blackboard {
  ...
  /** §11.2 (Wave 42, E6) — ring buffer, cap `MEMORY_CAP` (core/Memory.ts),
   *  oldest-first (pushMemory appends, shifts off the front once over cap —
   *  `recall`'s own "most-recent-first" ordering is applied at READ time,
   *  not by storing it backwards). Written by `Reasoner.ts` on a notable
   *  Activity SUCCESS and by `VisionSensor.ts`/`HearingSensor.ts` on a
   *  belief's first tick of existence — see `core/Memory.ts`'s own header
   *  for why those two are the only producers. Never persisted in a save,
   *  same as `lastScores`/`beliefs`: it is a live scratch trail for a future
   *  dialogue layer to query, not game state anything depends on. */
  memoryStream: MemoryRecord[];
}
