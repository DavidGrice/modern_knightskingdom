...
## 🏰 Phase 20 — The Kingdom of Instances *(the big one — next up)*
...
1. ✅ **Step 1 — Home = Template-09.** Shipped, and far cheaper than the original 7-system estimate by
...
2. ✅ **Step 2 — Evict the court** (King/Queen/Richard/John; Storm + Cedric follow in step 3). `NpcDef`
...
3. ✅ **Step 3 (relocations half) — every named character now lives in their instance.** Enemies at a
...
3b. ✅ **Step 3 (dressing half).** `CourtDressing.tsx` — real catalog set pieces, environment-placed like
   ...
   at John's landing.
4. ✅ **Step 4 — Location wayfinding (v1).** Quest completion now announces WHERE a newly revealed NPC
...
4b. ✅ **Location-exclusive side quests.** Two new errand kinds are location-bound *by mechanics*, not
   ...
   givers.
4c. ✅ **"Travel to X" main-quest beats.** Two new `QuestObjective` kinds — `'visit'` (credited by
...
5. **Step 5 — Persistence.** Claimed-plot building already works in template worlds; decide per-instance
...
1. ✅ **Per-instance minimap** — the map was hardcoded to the homestead frame (origin-centered at
...
2. ✅ **Positional mob audio** — `audio.playAt(name, x, z, vol)`: full volume ≤14 units from the player,
...
3. ✅ **Exclusive voice channel** — `audio.playVoice()/stopVoice()`: all 16 NPC voice-line call sites
...
**Standing audit list (apply the doctrine when touched):** ambience birdsong is identical in every
...
### 24A — Villager attribute system (customize each NPC's skill set)
...
### 24B — Real labor: villagers physically work and haul (no more auto-collect)
...
### 24C — Defender command system (a captain's orders)
...
## 🤝 Grok labeling pipeline — integration points (reviewed 2026-07-19)
...
consumable here:
- **`PAK_CAPABILITY_OVERRIDES.json`** — verified per-asset records (kind, displayName, wall roles
...
- **`reports/rigs/*_rig.json`** — verified per-shape → bone maps. The dragon (`l7517400/1`) has
...
- **`ORIENTATION_REGISTRY.json` / `PAK_ORIENTATION_CATALOG.json`** — per-asset correct eulers; use as
  ground truth whenever a new prop imports facing the wrong way.
- **`LEARNED_PART_LEXICON.json`** — family defaults (mc_wall → standable/connectable/destructible);
...
1. ✅ **Shipped v1 (2026-07-19)** — new **Prefabs** category (menu already had category tabs + search,
...
2. **Still to come**: arches/rounded generated pieces audit, the 4 Road models as pavement, the
...
1. ✅ **Other maps visible above the homestead — FIXED 2026-07-19.** Root cause was NOT the instance
...
2. ✅ **NPC equipment paperdoll + Armory (v1) — SHIPPED 2026-07-19.** A new homestead **Armory**
...
3. ✅ **Quest Log overhaul — SHIPPED 2026-07-20.** Replaced the old flat 11-entry list with
...
4. ✅ **Mobile-friendly pass v1 — SHIPPED 2026-07-20** (tech debt). Real, working touch controls, not a
...
5. ✅ **Real wall collision — SHIPPED 2026-07-20.** The mc-series prefab walls/towers were colliding
...
6. **GUI overhaul** ("boxy and square") — user will supply a reference GUI to learn from; hold for
   that, then a full visual pass.
7. ✅ **Beda & Alric now have a purpose — SHIPPED 2026-07-20.** They were pure flavor (a greeting,
...
8. ✅ **Station-specific crafting menus — SHIPPED 2026-07-20.** `CraftingPanel` now opens on a
...
## 📋 Remaining backlog (carried forward, grouped by theme)

**World & locations**
- Streams/rivers/waterfalls using the already-copied `spr203` cascade strip (natural fit for Phase 20 step 1).
- Sealed Crypt follow-ups: branching layouts, new objective types (escort/retrieve/survive), visual reskins
...
- The 4 unused `Road` models — pave travel routes / a marketplace square (fits Phase 20 step 3).
- Unused-asset audit completion: `Destructor` (4), second `Cannon`, 2 unused `Animal` models.

**Combat & content**
- **The Dragon** — ✅ *stage 1 shipped (2026-07-19)*: `DragonOmen.tsx` rolls 40% each deep night
...
- Halberd sweep / spear thrust as *player* weapons (molds already extracted and loaded).
- Armor tiers (iron → forged → castle-crested via torso decal variants).
- Catapult/trebuchet (only the cannon exists; firing sound `snd060` is waiting).
- Timed build challenges recreating the original game's six challenges (their full voiced texts survive in
  the extraction).
- Delivery quests — haul goods by cart between instances (perfect fit after Phase 20).
- Alliance follow-ups: reputation fallout with the court, alliance-exclusive quests/rewards, a turncoat path.

**Homestead & economy**
- **Bug (deferred, Phase 10 #9):** the traveling merchant's cart parks inside `BUILD_REGION` — relocate
  `MERCHANT_SPOT` (moot if Phase 20 re-sites the homestead anyway — fold it in there).
- Taming: falcon companion (`snd054`).
- Cooking depth beyond bread/cooked fish.
- ✅ **Unlockable crests** *(shipped 2026-07-19)* — went **account-level** instead of in-save:
...
- Dye recipes for new palette rows.
...
**Cast & AI**
- **Comprehensive AI/animation rig**: locomotion blending, idle variety, reactive behaviors — make the
  court and villagers read as alive rather than position-lerping props.
- Validate the user's forthcoming Grok-built item-catalog JSON against `BRICK_CATALOG.md` when it arrives.

**Styling**
- ✅ **Faction color-scheme overhaul (research + v1)**: real faction palettes extracted from the donors'
...
- ✅ **Dark-ages re-theme (v2, user-directed)**: the generic warm parchment-brown chrome is gone. The
...
**Tech (continuous)**
- **Performance & streamlining pass (requested 2026-07-18)** — a dedicated rendering-efficiency effort,
...
- Geometry LOD (asset-pipeline task: export the D1–D3 variants), rapier physics when ragdolls/siege demand
...
## 💡 New ideas under consideration (raised 2026-07-18 — design sketches, not yet committed)

- ✅ **Guilds (Phase 21, v1 shipped)**: five orders headquartered across the Kingdom of Instances — the
...
- ✅ **Skill tree (Phase 21, shipped)**: the Talent Tree in the Abilities panel — seven skill branches ×
  ...
  extends.
- **Perks with trade-offs (+/−)**: perks that cost something — "Berserker: +30% sword damage, −20% max
...
1. ✅ **Creator hands detached** — root cause was NOT parenting (hands correctly ride the arm joints):
...
2. ✅ **Spring cascade flowed UP** — visible pattern motion is opposite the UV offset drift; the cascade's
   `offset.y -= dt` scrolled the water upward. Sign flipped (brook was already correct).
3. ✅ **Villagers piled onto one bed / napped before beds existed** — night bed-seek now assigns one
...
4. ✅ **Defender patrol was static** — free-roaming defenders now walk a real circuit (radius 15 around
...
5. ✅ **Flower/herb nodes overlapped** — min-separation lock (≥8 units apart, ≥3 from trees, pond shore
...
6. ✅ **Weather bled across travel** — `Weather.tsx` resets the spell state machine on every destination
...
**Features:**
7. ✅ **One-stop tabbed menu** — `MenuTabs.tsx`: the six menu-family panels (Satchel I / Crafting C /
...
8. ✅ **Character callings (class system v1)** — `data/classes.ts`: 8 callings picked at creation
...
1. **User-reported batch above** — instance-bleed visibility ✅ and NPC equipment/Armory ✅ both
...
2. **Phase 25 wave 2** — remaining verified oc-series set pieces, Road pavement, wall-connection
   snapping via `wallRole`/`canConnectAsWall`.
3. **Phase 24 follow-ups** — per-defender orders, HUD order chip, deposit floaties, stall UI.
4. **Instance-separation audit list** (Phase 23 doctrine) whenever a listed system is touched.
5. Backlog alongside: trade-off perks, halberd/spear player weapons, armor tiers, dungeon follow-ups,
...
1. **Character Callings start too equipped.** "Squire" as a starting Calling collides with the EXISTING
...
2. **Start bare-handed entirely** — no starting axe, no calling kit items at all. (Verified this is safe:
...
3. **Panels visibly jump/resize** ("moving all over the place... especially quests") — `.game-panel`/
...
4. **Building has a visible hitch before the model appears**, and the SAME hitch happens on day→night
...
5. **"Press E to use" auto-opens the item immediately after building it** — likely the same interact
   prompt firing the instant construction completes, right where the player is already standing/aiming.
6. **Building placement should be hold-left-click** (like mining/chopping), not an instant single click,
   for better game feel and to avoid the jarring instant-pop building noted above.
7. **Need a "set active" for quests AND errands** — right now the HUD tracker auto-shows whatever the
   store computes as "the" active quest with no player choice, and side-quests/errands have no
   pinned/tracked state at all.
8. **Resource nodes "spam" multiple small pickups** — chopping/mining currently take several swings,
...
9. **Alric auto-spawn collision (real bug)** — `VILLAGER_NAMES` still lists `'Alric'` and `'Beda'` as
...
10. **Station interaction should open a FOCUSED quick-menu**, not the full tabbed Crafting book — walking
...
11. **Shield viewmodel is backwards** — right-click block in first person shows the lion face; should
    show the shield's BACK (the arm strap/hook), since that's what the wielder's own eyes would see.
12. **Dragon flies too far out** — the omen's flight path (±230 on X) exceeds `WORLD_HALF` (200), so it
...
13. **Villagers should only work set hours (5 AM – 8 PM)**, not 24/7 — `tickVillagers` currently has no
    time-of-day gating at all.
14. **HUD keyhint clutter** — the permanent top-right control reminder should move into the Satchel/
    inventory panel (or similar reference spot) instead of sitting on screen at all times.
15. **Defenders/villagers start fully armed for free** — contradicts the whole point of the new Armory:
...
### ✅ Quick/contained bugs shipped (9, 12, 11, 3, 14)

- **#9 Alric/Beda auto-spawn collision** — `VILLAGER_NAMES` (`src/game/data/villagers.ts`) no longer
...
- **#12 Dragon flight bounds** — `DragonOmen.tsx`'s flight `from`/`to` tightened from ±230/±140 down to
  ±160/±110–120, comfortably inside `WORLD_HALF` (200) so the omen no longer reads as leaving the map.
- **#11 Shield viewmodel orientation** — `Viewmodel.tsx`'s `BlockShield` rotation gained `+ Math.PI`
...
- **#3 Panel jump/resize** — added a `.game-panel.menu-family` CSS modifier (fixed `width`/`height`,
...
- **#14 HUD keyhint relocated** — the permanent top-right keybind box is gone from `HUD.tsx`; the same
...
### ✅ Medium-system bugs shipped (8, 13, 7)

- **#8 Resource-node harvest consolidated** — `harvestNode` (gameStore.ts) no longer decrements a
  ...
  meant to.
- **#13 Villager work hours** — added `WORK_START`/`WORK_END`/`isWorkingHours()` (`data/villagers.ts`,
...
- **#7 Set-active tracking for quests and errands** — new persisted `trackedQuest: 'main' | 'side'`
...
### ✅ Station quick-menu + preload/auto-open fixes shipped (10, 4, 5)

- **#10 Focused station quick-menu** — new `StationMenuPanel.tsx`: a small standalone contextual panel
...
- **#4 Upfront asset preloading** — new `game/preload.ts`: `preloadCommonAssets()` (called once from
...
- **#5 "Press E to use" no longer auto-opens the just-built item** — traced to a real bug: finishing
...
### ✅ Calling rework shipped (1, 2)

- **#1 "Squire" no longer collides with the earned rank** — the Calling's internal `id` stays `'squire'`
  ...
  Knight → Paladin).
- **#2 Every Calling starts completely bare-handed** — `data/classes.ts`'s 8 `ClassDef.kit` objects are
...
- Two real UI bugs this surfaced, both fixed: `CharacterCreator.tsx`'s "Begins with: {kit list}" line
...
### ✅ Hold-to-place building shipped (6)

- **Building placement now requires a deliberate hold (0.4s), mirroring mining/chopping's feel**, instead
...
- **The move tool (relocating an already-built piece) deliberately stays an instant click** — it has no
...
### ✅ Armory weapons shipped (15) — the big one, batch complete
...
- **A defender starts completely bare-handed** — `Defenders.tsx` no longer defaults `villager.loadout`
...
- **`setDefenderLoadout` now spends real Armory stock** instead of being a free toggle: a new
...
- **`'halberd'` is now a real, trackable `ItemId`** (it never was one before — purely an NPC-exclusive
...
- **VillagersPanel's Loadout row** now shows a "✋ Bare-handed" option (highlighted when actually
...
### ✅ Follow-up: item #6 actually meant the CONSTRUCTION step, not just placement
...
- **Construction is now driven by holding the ATTACK button (LMB)**, not E — matching the user's
...
- **`CombatController.tsx`'s mousedown handler skips the normal attack/ranged-fire logic** whenever
...
- **Gamepad/touch are deliberately NOT cut off** — they have no separate "click and hold" input mapped
...
1. **Black geometry pokes through the skybox when looking up**, and the broader ask: *"make separate
...
2. **Castle Wall cost imbalance** — `stonewall` (mc007) costs **6 stone** for an 8m segment while the
   `mc006` prefab Castle Wall costs **12 stone** for a *smaller* 7m segment. Same asset family, same
   role, double the price.
3. **Only one Castle Wall lives in the Walls category** (`stonewall`/mc007); every other wall-family
...
4. **Wall corner is far too small, leaving a grid gap** — ROOT CAUSE FOUND: the wall family is authored
...
5. **Trees/herbs spawn inside the build grid** — `seedNodes` claims "trees scattered outside the build
...
6. **Still can't run underneath walls — caught by the bbox edge.** The Phase-25 `WALL_CORE` split gave
...
7. **Opening the build menu moves the view off the player.** `BuildController`'s camera center is
...
8. **Defenders should keep the opposite schedule to workers** — sleep/stand down by day, patrol at
   night, since that's when skeletons rise. Workers already stop at 8 PM (`isWorkingHours`); defenders
   currently patrol 24/7.
9. **Killing an enemy should drop real random loot into the inventory**, and each NPC/enemy should
   carry some kind of lootable inventory rather than the current fixed `lootFor()` switch (skeleton →
   always 1 stone, etc.).
10. **The aerial build view should be angled with depth, not a flat 2D bird's-eye.** The user wants a
...
11. **The placement ghost needs a direction indicator** (arrow and/or wireframe) — right now the ghost
    is an untextured translucent box, so which way a piece faces is guesswork, which has caused real
    rotation mistakes.
12. **Allied-NPC appearance panel should match the player's own creator** — pick the villager's look
    (face/crest) and recolor arms/legs/hips, the same way `CharacterCreator` does for the hero.
13. **Homestead folk need diverse body types, including female.** `Villagers.tsx`, `Defenders.tsx` and
...
### ✅ Shipped from batch #2: walls, build grid, build view, villager identity (2, 3, 4, 5, 7, 10, 11, 13)

- **#4 + #2 + #3 — wall family unified.** Root cause was two scale factors: `walls`/`defense` entries
...
- **#5 — nothing but placed pieces inside the build grid.** `seedNodes` claimed the forest ring was
...
- **#7 — the build view opens where you're standing.** The camera centre was initialised to the region
...
- **#10 — the build view is raked, not a flat plan.** Still orthographic (that's what keeps a grid
...
- **#11 — the ghost shows which way it faces.** Added a full-size wireframe of the finished volume
...
- **#13 — the homestead is no longer one hardcoded male face.** `Villagers.tsx`, `Defenders.tsx` and
...
**Immediately useful, already applied:** `equipKind` cleanly separates donors with a weapon molded
...
**The big opportunity (not yet done — proposing as the next significant item):**
...
Also worth mining later: `traits.wall.destructionPhase`/`destructionPhaseCount` (a real, ordered
...
### ✅ Rig-lab part maps integrated (allied + enemy NPC rigging)
...
### ✅ Batch #2 complete — remaining items (8, 9, 12, 1, 6)

- **#8 — defenders keep the opposite shift.** New `isWatchHours()` (`data/villagers.ts`) — a window
...
- **#9 — enemies carry real, rolled inventories.** New `LOOT_TABLES` + `rollLoot()` (`combat.ts`):
...
- **#12 — villager appearance editor.** The roster's Equip panel gained an Appearance section with the
...
- **#1 — the black shape in the sky was the FALCON, not map bleed.** Enumerating scene geometry above
...
- **#6 — wall collision measured correct, and breaches are now walk-through.** Measured the actual
...
**Not yet integrated (the remaining lab surface):**
- `traits.minifig.swordHand` / `shieldHand` — the lab recorded which hand each
  donor actually holds things in; `Equipment.tsx` still passes a fixed side.
- Mount seat matrices (`DEFAULT_MINIFIG_HORSE_MOUNT.json` /
  `..._DRAGON_MOUNT.json`) and the four extra horse variants now copied in.
- Non-minifig prop rigs (drawbridge, catapult arm, jail cell, ladder,
  springboard) — `rigs/*_rig.json` has moving-part maps for these; they render
  static today.
- `ORIENTATION_REGISTRY.json` per-asset eulers (nothing currently looks wrong,
  so this is a correctness backstop rather than a fix).
- `traits.vehicle.canSeat` / `canDrive` / `canPush` — the siege pieces are
...
### Lab items closed out alongside

- **Handedness — verified, deliberately NOT wired.** `traits.minifig.swordHand`
...
- **Mount variants placed.** The lab charted six rideable horses where the game
...
### Lab backlog closed out (2026-07-25)
...
**3a · Title & Sign In** (Millennium Chrome) — moulded chrome plaque for the
...
**3b · Main Menu & Saves** (Metalheart) — two columns. The left rail keeps
...
**3c · Forge Your Hero** (Guild Leather) — all eight callings visible at
...
**3d · Options** (Metalheart) — two columns instead of one long scroll.
...
**How to Play / Credits** — re-shelled onto the same screen chrome, copy
...
**1a–1d · Field HUD** — nine clusters, each anchored to a viewport edge and
...
# MASTER PLAN — 2026-07-25 backlog (30 items)
...
1. **Build-exit spawn drift.** Leaving build mode drops the player south of
...
2. **Signpost sits inside the build grid.** **[found]** `SIGNPOST = {x:-14,
...
3. **12 missing thumbnails -> 404s.** **[found]** Every `/assets/props/lab/*.png`
...
4. **Placement arrow points the wrong way.** **[found]** `BuildController`
...
5. **Crossbow renders backwards in first person.** Viewmodel transform;
...
6. **Bolt/arrow damage too low.** Raise both, and make the longbow's draw
   fraction actually scale damage rather than only range.
7. **Villagers appear before their beds exist.** Arrival is gated on bed
...
8. **Character hitboxes.** Per-part capsules driven by the rig lab's own
...
9. **Projectiles stick instead of passing through.** Sweep the bolt/arrow
...
10. **Walk under archways — answering "can we ignore the bbox and use the
...
11. **Raider cart destroyable + turning wheels.** **[found]** `oc4806`/`oc4807`
...
12. **Rebuild the FPS view from the labelled minifig arms/hands.** Replace
...
13. **Live 3D character head in the vitals crest** (top-left HUD), from the
    player's chosen head donor, re-rendered when appearance changes; editable
    from the menu, not only at creation.
14. **Target readout while aiming** — name, allegiance, health bar and known
...
15. **Crosshair reads allegiance** — green on an ally, red on a hostile,
    neutral otherwise. Depends on E's allegiance model.
16. **Collection book panel** — everything scanned, with what is still
    unknown shown as gaps rather than hidden.

## E · Allegiance & the quest system  *(the largest single item)*

17. **Allegiance axis on the character.** Today there is only a one-way
...
18. **Comprehensive quest content across all three alignments**, with:
...
19. **Build grid rebuilt around real wall runs.** `GRID = 2` with
...
20. **Land purchase / homestead expansion.** Start on a small plot and buy
    outward; each expansion unlocks the resources standing in the newly
    claimed ground (trees, ore) *outside* the buildable zone.
21. **Grand Keep rebuilt from real parts** — the green MC00 base plate, four
...
22. **Import the remaining OBJ brick library.** `bricks.generated.json` has
...
23. **Towers and explosives auto-engage.** A deployed tower acquires the
    nearest hostile in range and fires on its own cadence; charges arm and
    detonate on proximity.
24. **Assign kin to emplacements.** Extend the existing defender-station
...
25. **Alric and Beda are mis-rigged** — heads and arms float or trail behind
...
**A1 · Build-exit spawn drift — fixed.** Root cause was not the build camera:
...
**A2 · Signpost moved out of the build grid.** It stood at (-14, 18), inside
...
**A3 · 12 missing thumbnails — fixed.** All nine siege engines and three
...
**A4 · Placement arrow rotation — fixed, verification partial.** The ghost
...
**A5 · Crossbow backwards in first person — fixed.** Same sign-ambiguity the
...
**A6 · Projectile damage raised.** Bolt 4 → **7** (battlement bonus now
...
**A7 · Villagers arriving before their beds — fixed.** `checkVillagerArrival`
...
**B8 · Per-part hitboxes, measured from the rig itself.** Ranged combat used
...
**B9 · Projectiles stick instead of passing through.** On impact a bolt
...
**B10 · Collision follows real geometry, not the bounding box.** Answering
...
**B11 · The raiders' ram can be broken, and its wheels turn.** The rig lab
...
**C12 · The first-person view now uses the player's OWN minifig arms.**
...
**Shared rig extraction first.** `lib/rigExtract.ts` replaces `lib/fpsArms.ts`
...
**D13 · The vitals crest is your actual head.** It was a generic helm glyph,
...
**D13b · Appearance is editable mid-game.** `AppearancePanel`, reachable from
...
**D14/D15/D16 share one resolve.** `game/targeting.ts` answers "what is the
...
# PLAN REVISION 2 — 2026-07-25 feedback folded in
...
**1 · The walls never reached the geometry-based collision. [found]**
...
**2 · The lab already names held weapons on 35 donors. [found]**
...
**3 · The horse rigs are fully verified. [found]**
...
- **E17 · Allegiance axis** on the character, persisted, surfaced in the
  Satchel stats. Unchanged from the original plan.
- **E18 · Quest content across good / neutral / evil**, with per-quest
  allegiance deltas, precursor chains, and locks that name their blocker.
  Unchanged.
- **E19 · NEW — the allegiance component itself.** Leo's banner at one end,
...
- **F19 · Build grid sized for real wall runs** (4-piece runs + corners tile
  end to end). Unchanged.
- **F20 · Land purchase / homestead expansion.** Unchanged.
- **F21 · Grand Keep from the MC00 green base + 4 corners + 2 middles.**
  Unchanged.
- **F22 · Import the remaining OBJ brick library.** Unchanged.
- **F23 · NEW — collision generator subdirectory fix. [found]** See finding
...
- **F24 · NEW — corner walls get no facing arrow.** They are bi-directional,
...
- **F25 · NEW — a real road to the signpost.** Lay road pieces from the
...
- **G23 · Towers and explosives auto-engage.** Unchanged.
- **G24 · Assign kin to emplacements.** Unchanged.
- **G25 · NEW — real pathfinding for everyone.** Enemies currently walk
  ...
  lean on it.
- **G26 · NEW — defenders engage the dragon.** They currently ignore a
  flying target entirely. Needs an air-target branch in the defender AI plus
  an arc that actually reaches altitude.
- **G27 · NEW — mounted patrols, captured horses and a stable.** Capture a
...
- **G28 · NEW — defender schedule bug.** Scouts/defenders are not sleeping
...
- **H29 · Weapons from the lab's named held gear.** Retire `weaponParts.ts`'s
...
- **H30 · Realign every first-person weapon.** With H29 landed, each weapon
...
- **H31 · Two hands in first person, raised.** The current single arm sits
  too low and the off hand does not exist. Both arms come from the same rig
  extraction the portrait uses.
- **H32 · Running arm animation.** Alternating arm swing while moving —
...
- **H33 · Bow draw-back animation**, and the crossbow's equivalent (span and
  load). The longbow already tracks a draw fraction for damage; it has no
  visual.
- **H34 · Pickaxe and hammer re-alignment.** No lab mold exists for either
...
- **H35 · Alric and Beda's rigs.** Their heads and arms scatter while they
...
- **H36 · Horse animation.** Grazing horses are frozen despite verified rigs
  (finding 3). Wire `leg_upper`/`leg_lower` into a walk/graze cycle. Feeds
  G27's mounted patrols.
- **H37 · Raider cart wheel wobble.** The wheels turn on the wrong axis or
...
- **I38 · Replace the loading screen.** Still the old brick art and
  "Raising the drawbridge…". Should match the front-door design language the
  UI pack established.
- **I39 · Enemy health above the model, in the world.** Not a modal, and not
...
- **I40 · No readout for friendly NPCs at their posts.** Alric and Beda
...
- **I41 · Command panel: text overflow and pointer-lock churn.** Text runs
...
- **I42 · Richard's fourth line is King Leo. [found]** `lore_richard_cedric`
...
- **I43 · XP scaling rework.** Current awards do not scale sensibly across
  the level range.
- **I44 · Level-ups raise vitals.** Each general level grants a small
...
**F23 · The walls read their real geometry now — no Blender needed.**
...
**G25 · A\* pathing, built on those volumes.** `game/navgrid.ts` rasterises
...
**I40 · The aim card is an aiming readout again.** Confirmed from your
...
**E17 · The allegiance axis.** The game had `alliance` — a one-way pledge to
...
**E19 · The house-banner meter.** Cedric's banner at one end, Leo's at the
...
**E18 · Errands that take a side.** `data/allegianceQuests.ts` adds three
...
**H29 · Weapons come from the lab's verified role maps now.**
...
**H30 · Every weapon re-seated.** With one convention (grip at origin, length
...
**H31/H32 · Two hands, raised, and a real run swing.** The off hand exists
...
**H33 · The bow draws.** Real mold, real nocked arrow riding the string back,
...
**H34 · Pickaxe and hammer.** Stated plainly: **there is no mold for either
...
**H35 · Alric and Beda.** `Npc.tsx` passed `keepProps` for every court NPC,
...
**H36 · The horses move.** The lab verified a full four-leg rig on all six
...
**H37 · The cart wheels roll instead of wobbling.** A wheel is a thin disc and
...
**F19 · The grid is sized so a castle actually tiles.** A straight wall is 8m
...
**F20 · Those sizes ARE the land you buy.** Rather than a separate mechanic
...
**F22 · The last two folders reached the catalog.** `buildings` (the rest of
...
**F24 · Corners no longer claim a facing.** A corner turns a run either way,
...
### Still open in F
- **F21 · The Grand Keep from the MC00 green base + 4 corners + 2 middles.**
...
- **F25 · The road to the signpost.** Not started.

---

# PLAN REVISION 3 — the brick economy and build feel
...
**J45 · Bricks become the resources.** Right now you mine a rock and receive
...
**J46 · Resource grounds, unlocked by the deed.** Gathering should happen in
...
**J47 · A real ghost, not a blanket square.** The placement preview is a
...
**J48 · Buildings rise out of the ground.** When construction completes the
...
**J49 · Building takes real work.** A couple of hammer swings is not a
...
**J50 · Flames, animated from the lab's own parts.** The lab names
...
**J51 · The Grand Keep, composed** *(was F21, restated to your design)*.
...
**A real road out of the homestead.** The signpost stood in open grass: you
...
**I38 · The loading screen.** It was a line of italic text on the old panel
...
**I42 · Richard's fourth line was King Leo's.** "Did you keep up alright? …
...
**I43 · XP scaling — the actual problem, and a fix that does not re-rank
...
**I44 · Levelling is felt.** Every general level now adds to both vitals: a
...
### Still open in I
- **I39 · Enemy health as an in-world billboard** above the model rather than
  the aim card. Not started.
- **I41 · Command panel** text overflow and the pointer-lock churn. Not
...
**Vitals are a bar, not hearts.** The HUD work replaced the heart glyphs with
one vigour bar, but the language never followed — food said "+3 ❤", the
satchel said "restoring hearts". All of it now says vigour.

**I39 · Enemy health, in the world.** A billboarded plate above each wounded
...
**I41 · The order panel is a hold-to-open radial.** Two problems, one cause:
...
**G23/G24 · Emplacements fight, and posted kin matter.** Anything the lab
...
**G26 · Defenders engage the dragon.** Nothing on the ground had an air
...
**G28 · Scouts sleep.** The rest branch only ran for `patrol`. Scout is a
...
# BLOCK K — defects in what was just shipped
...
**K52 · Horse textures and colours are wrong.** Moving the horses from the
...
**K53 · The grazing animation is broken. [found]** Only the upper legs move.
...
**K54 · Ally NPCs still do not RIDE.** G27 gave a mounted defender speed and
...
**K55 · The Stable is a wall. [found]** I pointed it at `mc008`, which is a
...
**K56 · FPS weapons still sit wrong, and the bow is reversed.** The arrow
...
**K57 · Beda's arms come from a different body. [found]** Her config is
...
**K58 · Crenellated walls still cannot be walked under. [found]** F23 gave
...
**K57 · Beda's arms.** Her config had `headDonor: minifiggenericgood00` with
...
**K55 · The Stable was a wall.** It pointed at `mc008`, a straight wall
...
**K52 · Horse textures.** Moving the horses to the OBJ path also moved their
...
**K53 · All four legs move.** `splitDisjoint` merged parts within a flat 8
...
**K56 · The bow and arrow were swapped.** Both sat inside one rotated group,
...
### K58 · Investigated, and the answer is not a code fix
...
**K59 · The road is a road now.** The four plates were confirmed wrong by
...
**K54 · Allies ride, visibly.** A defender with a stabled horse assigned now
...
**K60 · The sky sat 80 m too high.** The skybox was pinned at a fixed
...
**K61 · The target plate hangs over their head.** It was a 210px panel parked
...
**The Next.js dev badge is gone** (`devIndicators: false`) — it sat over the
...
**J47 · A real ghost.** A site used to be a translucent gold BOX with a solid
...
**J48 · It rises out of the ground.** Under the plan sits the real solid model,
...
**J49 · Building takes real work.** The swing count topped out at 8, so a keep
...
**J50 · Fire, and it is the game's own.** Following the lab's `flame` roles
...
**J45 · Bricks ARE the resources.** Gathering yields a specific catalogue
...
**J46 · Resource grounds, held by deed.** Every node now seeds inside a NAMED
...
**J51 · The Grand Keep, composed.** The keep is no longer a mesh you drop.
...
### Carried forward out of J
- **No baseplate mesh exists.** The extraction has no large green base plate
...
- **J45 stops at the resource families.** Five materials map to five real
...
**L63 · Wall collision is on the wrong side. [likely rotation]** Standing
...
**L64 · Not every wall piece counts as a wall.** Building plain wall sections
does not advance the wall-sections quest. Every piece in the wall family has
to classify as one.

**L65 · Walls snap to opposite edges depending on facing.** A wall placed on
...
**L66 · Allied NPCs work from impossible distances.** A villager will mine,
build or attack a target hundreds of metres away. They must be adjacent before
the action starts.

**L67 · Guards patrol through the day.** A defender on patrol keeps patrolling
in daylight. Patrol is a NIGHT order: by day they should sleep.

**L68 · The south furniture is in the way.** The merchant should stand at the
...
**L69 · Buying land has no handle in the world.** You should be able to walk
...
**L70 · The resource grounds sit too close.** J46's circles overlap the
homestead. They need spreading out.

**L71 · The road pieces — I was wrong. [found]** K59 rejected `l4109610–13` as
...
**L72 · Buildings need a menu, and walls need an upgrade slot.** Clicking a
...
**L73 · The FPS weapons are still misaligned.** H29/K56 improved the grip
...
**L71 · The road, from the real plates.** K59's rejection of these four pieces
...
**L64 · Every wall is a wall.** "anywall" was hardcoded to the two pieces that
...
**L63 · Collision turned the wrong way. [found]** `collisionBoxesFor` rotated
...
**L65 · Rotation no longer moves the wall face.** A crenellated wall is a
...
**L66 · Work happens where the worker is.** The trip timer ran on the clock
...
**L67 · The watch keeps the night shift.** Standing down by day was written
...
**L70 · The grounds are clear of the homestead.** Pushed out, and the layout
...
**L68 (part) · The merchant is off the build edge.** It stood at (36, 16),
...
**L69 · Deeds are bought on the ground they buy.** Every ground's boundary
...
**L62 · You can see the horse you are riding.** Mounting hid the meadow
...
**L72 · Buildings have a menu.** A click used to pick the piece up, unasked —
...
**L73 (part) · The bow is in the right hand now — the left one.** It was
...
### Still open in L
- **L73 (rest) · the arrow on the string** still lies vertically alongside the
...
- **L62 (rest) · lance and halberd from the saddle.** Ranged works; the lance
  still only exists inside the joust mechanic, and the halberd has no mounted
  pose.
- **L68 (rest) · The south guard posts.** I could not identify which props
...
## Future · build from the real instruction sets
...
# The workshop · findings from `kk_research_folder`, 2026-07-26
...
**1. The game's models are NOT brick-accurate, and that is decisive.**
...
**2. But the game ships each set already broken into sub-assemblies.**
...
## So there are two different products here, not one

**A · The Assembly Workshop** — build a set from its sub-assemblies and their
...
**B · The Instruction Build** — the real retail set, brick by brick, faithful
...
## Coordinate note, carried from the research README
...
# Block M — the Assembly Workshop, shipped 2026-07-26
...
# Proposed · From a homestead to an empire
...
- **`PlacedBuilding.world`** — the Phase 23 instance-separation doctrine. A
...
- **`WORLD_DESTINATIONS`** — nine template worlds already exist as travelable
...
- **The deed ladder** (F20, and J46's grounds) — buying rights to ground is
...
**Suggested first slice**, if this is wanted: take ONE destination — the Far
...
# Empire design — answered, 2026-07-26
...
**New, and small enough to do on its own:** a villager who joins the homestead
...
**Blocked meanwhile:** the remaining bricks and the template maps are not fully
...
- **Anything that depends on what a template world CONTAINS.** The remaining
...
- **Option B of the workshop** (instruction-accurate builds): still needs
...
## Blocked on a decision or a pointer
- **The south guard posts.** The merchant cannot be put on them until I know
  which props they are; the only fixed structures nearby are Alric's and
  Beda's houses.
- **The road's route vs. southward expansion.** Small change, but it wants one
  pass over the whole southern layout rather than a nudge.

## Unblocked, large, and not started
...
- **Per-world villager labour.** The empire's foundation: settlements have to
...
- **Generalised interiors.** Quests come from allied NPCs indoors, so ordinary
...
- **L73 (rest) · weapon poses.** Only the bow's HAND was wrong and that is
  ...
  than more nudging.
- **L62 (rest) · lance and halberd from the saddle.**
...
**Corner plate turned 180°** — it ran south-to-east and wanted west-to-north.

---

# BLOCK N — playtest round, 2026-07-26

**N74 · Herbs never come back.** A foraged herb patch does not re-render after
...
**N75 · Fence the grounds instead of drawing lines.** The gold boundary strips
...
**N76 · Quests and errands should pay GOLD as well as resources.** Right now
they pay materials only, which makes the deed ladder (and now village
purchase) hard to feed.

**N77 · The merchant's hands float.** Same fault Alric and Beda had when they
first spawned — arms and hands from a donor whose body type does not match
(see K57). Audit the merchant's config the same way.

**N78 · The merchant should live somewhere and TRAVEL.** A small walled place
...
**N79 · Enemies should come UP THE ROAD.** Bandits and raiders spawning in
...
**N80 · Guard shifts.** If raids can come by day, the watch cannot all sleep
...
**N81 · The FPS hands are gesturing backwards.** Chopping bare-handed moves the
...
**N82 · One readout for a target, not two.** There is a health bar AND a card
